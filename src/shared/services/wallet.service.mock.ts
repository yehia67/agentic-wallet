...
export class WalletService {
  ...
  async approveToken(spender: string, amount: bigint = BigInt('0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff')): Promise<string> {
  ...
  async batchTransactions(transactions: Array<{ to: string; value?: bigint; data?: string }>): Promise<string> {
    this.logger.log(`Mock batch transactions: ${transactions.length} transactions`);
